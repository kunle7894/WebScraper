...
public class ForexFactoryFrame extends Frame
{
	...
	private JLabel directory, excelFile, dataCollect, webName;
	...
	private MultiWebsiteGetterForex forexSpecGetter;
	...
	public ForexFactoryFrame(Changer change)
	{
		changer = change;
		...
		contents = new Object[][]
							  {
								...
								{"Time", "before<td class=\"calendar__cell calendar__time time\">"},
								{"Currency", "<td class=\"calendar__cell calendar__currency currency \">"},
								{"Event Title", "<span class=\"calendar__event-title\">"},
								{"Forecast", "<td class=\"calendar__cell calendar__forecast forecast\""},
								{"Actual", "<td class=\"calendar__cell calendar__actual actual\""},
								{"", ""},
								{"", ""},
								{"", ""},
								{"", ""},
	...
	public void writeExcel()
	{
		...
		{
			JOptionPane.showMessageDialog(frame, excelWriter.getError(), "Input Error",
											JOptionPane.ERROR_MESSAGE);
		}
		else
		{
			JOptionPane.showMessageDialog(frame, "Finishied Without Problems!", "Input Error", 
	...
	public void createForexScraper()
	{
		ArrayList<String> vals = new ArrayList<String>();
		vals.add("<td class=\"full calendarspecs__specdescription\">");
		vals.add("<td class=\"actual calendarhistory__row calendarhistory__row--actual\">");
		forexSpecGetter = new MultiWebsiteGetterForex(vals, url, "res/chromedriver.exe", dialogBox);
		names.add("Data Forecast");
		names.add("Previous 1");
		names.add("Previous 2");
		names.add("Previous 3");
	}
	...
	public void addInfoExcel()
	{
		...
		{
			webInfo.get(i).addVal(0, names.get(infoTogether.size())); //Gets the first Name 
			//value to add to the beginning of the Arraylist
			infoTogether.add(webInfo.get(i).getValues());
		}
		
		webInfo = forexSpecGetter.getWebStorage();
		for (int i=0; i<webInfo.size(); i++)
		{
